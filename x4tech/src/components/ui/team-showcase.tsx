...
export default function TeamShowcase({ members = DEFAULT_MEMBERS, onMemberClick }: TeamShowcaseProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        />
        <span className={cn('text-base md:text-[18px] font-semibold leading-none tracking-tight transition-colors duration-300', isActive ? 'text-white' : 'text-white/80')}>
          {member.name}
        </span>
      ...
      <p className="mt-1.5 pl-[27px] text-[9px] md:text-[10px] font-medium uppercase tracking-[0.2em] text-white/55">
        {member.role}
      </p>
    </button>
  );
